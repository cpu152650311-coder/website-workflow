...
name: 建站工作流
version: "11"
description: 电子制造B2B出口建站。一句话→完整页面树→SEO+安全基础设施→全站图片策略→10秒买家测试→部署后审计。产出huaxingpcba级完整度（55+页），纯静态HTML多页站。博客只需3篇。
...
| 品类 | 典型子能力展开 |
|------|-------------|
| PCB | pcb-fabrication/{standard,quick-turn,impedance-control,back-drilling} / pcb-types/{aluminum,ceramic,flex,hdi,heavy-copper,high-speed,rf,rigid-flex} / advanced-pcb/{via-in-pad} |
| PCBA | pcba-assembly/{smt,through-hole,bga-qfn,conformal-coating,functional-testing} / box-build/{enclosure-integration,cable-harness,testing-packaging} |
| SMT | smt-stencil/{laser-cut,step-stencil} + components-sourcing |
...
- **版权区格式强制**：`© 2026 {Brand Name} — Shenzhen, China` 或对应城市
