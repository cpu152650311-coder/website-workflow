...
export CLOUDFLARE_ACCOUNT_ID=$(grep CLOUDFLARE_ACCOUNT_ID ~/.hermes/.env | cut -d= -f2)
```
...
# 3. 如果仍显示旧内容：CDN 传播延迟，sleep 5-10s 后重试
```
...
JS：`openModal()` / `closeModal()` + 点击遮罩关闭。CSS 在 design-tokens.css。

> **踩坑经验**：跳转 `/contact/` 页面打断浏览流程，弹窗询盘转化更好。
