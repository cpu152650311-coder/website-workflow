...
用户: "做个{品类}外贸站，叫{品牌名}"
  │
  ▼
Phase 0  Agent 自动行业调研 → 菜单勾选（能力/行业/Logo方向/设计方向）
  │ 🛑 用户确认菜单
  ▼
Phase 1A  Logo 生成（3-4 候选）→ 用户选定 → 转换 favicon
  │ 🛑 用户确认 Logo
  ▼
Phase 1B  配色方案（2-3 组）+ 字体方案 → 用户选择
  │ 🛑 用户确认配色+字体
  ▼
Phase 1C  图片 prompt 审阅（5-8 个关键 prompt）→ 用户确认方向
  │ 🛑 用户确认 prompt
  ▼
Phase 2   样图生成（3 张）→ 展示 → 🛑 确认 → 全量生图
  │ quality 写死 low，零复用
  ▼
...
```
Phase 0  主 Agent → 行业调研 + 菜单勾选（🛑用户确认）
         ├─ content-blueprint.md（产品参数/认证/交期/产能/工艺/信任信号）
         └─ page-tree.json（4层页面树）

Phase 1A Agent → Logo 生成（3-4候选）+ Favicon 转换（🛑用户确认）
Phase 1B Agent → 配色方案（2-3组）+ 字体 + 设计基调（🛑用户确认）
Phase 1C Agent → 图片 prompt 审阅（5-8关键prompt）（🛑用户确认）
         ...
         └─ generated/logo.webp, favicon.ico, apple-touch-icon图标

Phase 2  GPT Image 2 → 样图3张 → 🛑确认 → 全量生图（quality 硬写死 low）
...
### Phase 1A：Logo 生成 + Favicon

Agent 根据 Phase 0 用户选择的 Logo 方向，写出 3-4 个 prompt 变体（同一方向的不同表现），**并行生成 3-4 张 logo 候选**：

```
python gen-site-images.py --prompts logo-variants.json --out ./generated --quality low
```

> `logo-variants.json` 临时文件，包含 3-4 个同方向的 logo prompt 变体。质量硬写死 `low`。

生成后，**展示给用户选择**。用户选定一张后，转换为 favicon 格式：

```bash
python gen-favicon.py --source ./generated/logo.webp --out ./generated
```

> `gen-favicon.py` 位于本技能 `scripts/` 目录。Agent 通过 `skill_view` 获取源码写入项目根目录。自动调用 Pillow 生成 favicon.ico / apple-touch-icon / icon-192 / icon-512。

**输出**：`generated/logo.webp`, `favicon.ico`, `apple-touch-icon.png`, `icon-192.png`, `icon-512.png`
...
Agent 提取该品类的视觉特征后，**呈现选项让用户选择**，不替用户做决定。
...
Agent 基于品类特征，设计 2-3 组不同的配色方案，每组包含：
