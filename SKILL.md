...
> **v11.1**：**视觉设计专业化 + Logo自动化 + 去硬编码**。Phase 1B 整合 ui-ux-pro-max 获取专业配色/字体建议（161 调色板 + 57 字体配对），新增 Pinterest 视觉参考搜索获取品类真实 mood board。Logo 生成流程全自动：GPT Image 2 生图 → rembg AI 去背景 → Pillow 缩放 favicon 全尺寸。gen-site-images.py 新增 `--limit N` 参数支持样图阶段精确控制。Phase 0 研究增强：WebSearch + WebFetch 多维度搜索 + 竞品站信息架构分析。**去硬编码**：Hero CSS 代码块所有色值替换为 CSS 变量（`--hero-text/--hero-overlay/--accent-alpha` 等），页面骨架表从「固定板块名+顺序」改为「说服功能映射」，禁止照搬示例值及工作空间已有站点的 CSS/HTML。
...
> ⛔ **禁止照搬骨架的 Section 名称/数量/顺序**。以下骨架描述的是「每个页面要承载的说服功能」——Agent 必须根据具体品类的 buyer journey 重新组合、重命名、甚至合并/拆分这些说服功能。
> 同一品类、同一骨架 → 但不同的品牌人格、不同的配色、不同的排版 → 产出完全不同的页面。
> **任何来自 huaxingpcba 或其他工作空间站点的具体 CSS/HTML/配色/板块组合都不得复用。**

| 页面类型 | 说服逻辑（功能映射，非固定板块） | Agent 自主决策 |
|----------|------------------------------|:--:|
| 首页 | 品类定位(1秒识别) → 信任信号(凭什么信) → 能力展示(你能做什么) → 数据论证(规模/品质) → 转化路径(CTA) | 这些说服功能如何组合成板块、板块数量、命名、顺序——全部自主 |
| 能力 Hub | 分类导航 → 快速定位 → 信任 → 转化 | 同上 |
| 能力详情 | 技术参数 → 工艺能力 → 对比优势 → 相关能力 → 转化 | 同上 |
| 行业 Hub | 行业覆盖 → 差异优势 → 转化 | 同上 |
| 行业详情 | 痛点共鸣 → 技术要求 → 方案呈现 → 证据 → 案例 → 转化 | 同上 |
| 关于/工厂/质量 | 数据先声 → 内容展开 → 认证墙 → 转化 | 同上 |
| FAQ | 问题分组 → 点击展开 → 转化 | 同上 |
| 联系 | 表单优先 → 补充信息 → 转化 | 同上 |
| How It Works | 流程可视化 → 打消顾虑 → 转化 | 同上 |
| Design Guides | 知识价值 → 实用工具 → 转化 | 同上 |
| 博客列表 | 内容索引 → 阅读引导 → 转化 | 同上 |
| Google Ads 落地页 | 每屏一个说服点 → 表单采集 → 移动端 sticky CTA（具体段落数量按品类决定） | 同上 |
...
> ⚠️ **以下 CSS 为结构骨架参考，不是可复制的模板。**
> 所有颜色值（背景色、文字色、遮罩色、强调色）必须从 `design-tokens.css` 的 CSS 变量中引用。
> 示例中的具体色值（如 `#0a0f1a`、`rgba(200,150,62,0.25)`、`#fff`）**仅作占位演示，禁止照搬**——
> 每个站的颜色体系由 Phase 1B 的 ui-ux-pro-max + 用户选择唯一确定，不存在默认配色。
>
> **明暗主题 overlay 规则**：
> - 暗色主题：`::before{background:rgba(0,0,0,0.4~0.5)}`（暗遮罩加深背景）
> - 亮色主题：`::before{background:rgba(255,255,255,0.15~0.3)}`（白遮罩提亮背景）或省略 overlay
> - 遮罩颜色和透明度取决于背景图明暗和文字可读性需求，不存在统一的 `.45` 值

#### 默认布局：居中

居中 Hero 是 B2B 出口站最常见的布局选择：

```css
/* ⚠️ 结构骨架 — 所有颜色变量在 design-tokens.css 中定义，禁止照搬示例色值 */
...
.hero{
  ...
  background:var(--bg-hero-fallback);      /* ← design-tokens.css 定义，与品牌色匹配 */
  overflow:hidden;
}
.hero::before{
  content:'';position:absolute;inset:0;
  background:var(--hero-overlay);          /* ← design-tokens.css 定义; 暗色主题=rgba(0,0,0,x); 亮色主题=rgba(255,255,255,x) */
  z-index:1;
}
.hero-img{
...
.hero-content .section-label{
  background:var(--accent-alpha);          /* ← design-tokens.css 定义 */
...
.hero-content h1{
  ...
  line-height:1.1;
  color:var(--hero-text);                  /* ← design-tokens.css 定义; 暗色主题=浅色; 亮色主题=深色 */
}
.hero-content h1 span{color:var(--accent-glow)}
.hero-content .hero-sub{
  font-size:var(--text-lg);
  color:var(--hero-subtext);               /* ← design-tokens.css 定义 */
...
.hero-stats{
  ...
  border-top:1px solid var(--hero-border);  /* ← design-tokens.css 定义 */
}
.hero-stat-value{font-size:var(--text-3xl);font-weight:700;color:var(--hero-text);line-height:1}
.hero-stat-label{font-size:var(--text-sm);color:var(--hero-subtext);margin-top:4px}
...
**Agent 使用此骨架的正确方式**：
1. 从 `design-tokens.css` 读取所有颜色变量名
2. 在 CSS 中用 `var(--xxx)` 引用，不写硬编码色值
3. 结构（居中/靠下/左对齐）由品牌人格三轴决定
4. 间距、字号继承了 `design-tokens.css` 的变量系统
...
- `hero-overlay` 的透明度取决于背景图的明暗——先看图再定值，不存在统一公式

#### 备选布局：靠下（适合车间广角图）

如果 Hero 图是车间广角（非产品微距），可将文字靠下放置。Overlay 用渐变（上透下浓）而非均匀色：

```css
/* ⚠️ 渐变值按实际背景图明暗调整，禁止照搬 */
.hero{align-items:flex-end;padding-bottom:var(--space-4xl)}
.hero::before{
  background:linear-gradient(to bottom,
    var(--hero-overlay-top) 0%,      /* 上半部几乎透明：透出车间场景 */
    var(--hero-overlay-mid) 30%,     /* 中间过渡 */
    var(--hero-overlay-bottom) 60%,  /* 下半部加重：保证白色文字可读 */
    var(--hero-overlay-max) 100%);   /* 最底部最浓 */
}
```

> Overlay 具体色值在 `design-tokens.css` 中定义为 `--hero-overlay-top/mid/bottom/max` 四个变量。
> 暗色主题用 `rgba(0,0,0,x)`，亮色主题用 `rgba(255,255,255,x)`。
> 渐变的「透→浓」比例取决于背景图上半部的内容密度——先看图再调值。
